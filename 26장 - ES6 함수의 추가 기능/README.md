...
---
## 26.4 Rest 파라미터
화살표 함수에서 arguments 객체를 사용할 수 없는데 가변 인자 함수를 구현해야할 때 Rest 파라미터를 사용함
...
foo(1,2,3);
```
- Rest 파라미터는 마지막 파라미터여야함!
- 하나만 선언 가능
- length 프로퍼티에 영향주지 않음

## ES5
`arguments` 객체는 유사 배열 객체이므로 배열 메서드를 사용하려면 `Function.prototype.call` 이나 `Function.prototype.apply` 메서드를 사용해 `arguments` 객체를 배열로 반환해야하는 번거로움이 있었음
...
## ES6
`rest파라미터`사용하여 가변 인자 함수 목록을 배열로 직접 전달받을 수 있음.
...
console.log(sum(1,2,3,4,5)); //15
```
ES6 메서드는 Rest파라미터와 arguments 객체를 모두 사용할 수 있음. 하지만 화살표 함수는 함수 자체의 arguments 객체를 갖지 않음

---
## 26.5 매개변수 기본값
매개변수에 인수가 전달되지 않은 경우 기본값을 할당할 필요가 있음(방어코드)<br>
🍀 인수 전달하지 않은 경우, undefined (null은 null반환)
...
console.log(sum(1)); //1
```
ES6에서 도입된 매개변수 기본값을 사용하면 함수 내 인수 체크 및 초기화 간소화 가능
```javascript
function sum(x = 0, y = 0) {
    return x + y;
}
```
매개변수 기본값은 length 프로퍼티와 arguments 객체에 아무런 영향을 주지 않음